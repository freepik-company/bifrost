...
IMG ?= ghcr.io/freepik-company/bifrost:placeholder
...
build: fmt vet check-go-target ## Build CLI binary.
	go build -o bin/bifrost-$(GOOS)-$(GOARCH) cmd/main.go
...
package: check-go-target ## Package binary.
	...
	@if [ "$(OS)" = "linux" ]; then \
		tar --transform="s/bifrost-$(GOOS)-$(GOARCH)/bifrost/" -cvzf dist/$(PACKAGE_NAME) -C bin bifrost-$(GOOS)-$(GOARCH) -C ../ LICENSE README.md; \
	elif [ "$(OS)" = "darwin" ]; then \
		tar -cvzf dist/$(PACKAGE_NAME) -s '/bifrost-$(GOOS)-$(GOARCH)/bifrost/' -C bin bifrost-$(GOOS)-$(GOARCH) -C ../ LICENSE README.md; \
	else \
